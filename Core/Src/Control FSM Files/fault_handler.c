...
//if compressor is supposed to be running but the temperature sensors are not detecting any cooling trigger fault
	...
	if (compressor_fault_state = true) {
		compressor_fault_handler();
	}
}
...
void set_rgb_fault_lights()
{
// set the RGB LED to run the pattern that corresponds to each of the fault types
}
